...
[
\mathbf{Loc}{\Lambda}(X) ;\simeq; \pi_1(X, x_0)\text{-}\mathbf{Mod}{\Lambda}^{\text{f.g.}}
]
...
Furthermore, for a finite ring (\Lambda) (where (n\Lambda = 0) for some (n \in \mathcal{O}X^\times)):
[
\mathbf{Loc}{\Lambda}(X) ;\simeq; \pi_1^{\text{et}}(X, \bar{x}0)\text{-}\mathbf{Mod}{\Lambda}^{\text{f.g., cont}}
]
...
[
\begin{aligned}
\text{Rank } n \text{ }\Lambda\text{-local systems} &;\Longleftrightarrow; \mathrm{GL}n(\Lambda)\text{-torsors} \
&;\Longleftrightarrow; H^1{\text{et}}(X, \mathrm{GL}n(\Lambda)) \
&;\Longleftrightarrow; \operatorname{Hom}{\text{cont}}(\pi_1^{\text{et}}(X, \bar{x}_0), \mathrm{GL}_n(\Lambda))
\end{aligned}
]
...
[
(R^i f_* \Lambda){\bar{y}} ;\cong; H^i{\text{et}}(X_{\bar{y}}, \Lambda)
]
